...
const BTN_GRID_SIZE_ID = 'grid-size';
const BTN_SPEED_1_ID = 'speed-1';
const BTN_SPEED_2_ID = 'speed-2';
const BTN_SPEED_10_ID = 'speed-10';
const BTN_SPEED_100_ID = 'speed-100';
const BTN_SAVE_ID = 'save-game';
const BTN_RESUME_ID = 'resume-game';
const GENERATION_VAL_ID = 'generation-value';
const GRID_SIZE_VAL_ID = 'grid-size-value';

function getGridActiveCells(gridManager) {
  const activeCells = [];
  const grid = gridManager.grid;

  for (let x = 0; x < grid.length; x++) {
    for (let y = 0; y < grid[x].length; y++) {
      if (gridManager.isActiveCell(x, y)) {
        activeCells.push(`${x},${y}`);
      }
    }
  }
...
function computeNextGeneration(gridManager, generation) {
  // Exercice 5
  generation += 1;
  document.getElementById(GENERATION_VAL_ID).innerText = generation;
...
function main() {
  ...
  const resetGrid = (initialState) => {
    ...
    // Exercice 4
    document.getElementById(GRID_SIZE_VAL_ID).innerText = gridSize;
    ...
    gridManager.setInitialState(initialState || INITIAL_STATE);  
  };
  ...
  const autoplay = () => {
    ...
    }, GENERATION_INTERVAL/speedFactor);
  };
  ...
  // Exercice 4
  document.getElementById(BTN_GRID_SIZE_ID).addEventListener('click', () => {
    const result = prompt('Quelle est la taille de la grille', '30');
    ...
    if (isNaN(value) || value < 30) {
      throw new Error('Value must a number at least 30');
    }
  ...
  // Exercice 6
  document.getElementById(BTN_SPEED_1_ID).addEventListener('click', () => { 
    speedFactor = 1;
    autoplay();
  });
  document.getElementById(BTN_SPEED_2_ID).addEventListener('click', () => {
    speedFactor = 2;
    autoplay();
  });
  document.getElementById(BTN_SPEED_10_ID).addEventListener('click', () => { 
    speedFactor = 10;
    autoplay();
  });
  document.getElementById(BTN_SPEED_100_ID).addEventListener('click', () => { 
    speedFactor = 100;
    autoplay();
  });
  ...
  document.getElementById(BTN_SAVE_ID).addEventListener('click', () => { 
    const activeCells = getGridActiveCells(gridManager);
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ activeCells, gridSize }))
  });

  // Exercice 8 
  document.getElementById(BTN_RESUME_ID).addEventListener('click', () => { 
    ...
    gridSize = parsedContent.gridSize;
    resetGrid(parsedContent.activeCells);
  });
}
